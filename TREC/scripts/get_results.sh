...
for i in "${!stemmer[@]}"
do
	./bin/atire -t"${stem[$i]}" -RBM25:"${k1[$i]}":"${b[$i]}" -findex $TREC_PATH/TREC\-$SUB/TREC\-$SUB.aspt -q$TREC_PATH/TREC\-$SUB/topics.$LOW\-$HIGH -a$TREC_PATH/TREC\-$SUB/$LOW\-$HIGH.qrels -et -o$TREC_PATH/output/TREC\-$SUB\-"${stemmer[$i]}".out
done
...
do
	...
	./trec_eval $TREC_PATH/TREC-$SUB/$LOW\-$HIGH.qrels $TREC_PATH/output/TREC\-$SUB\-$s.out -q -m map | cut -f 2,3 >> $TREC_PATH/map/tuned_map_TREC\-$SUB\-temp
	while read -r line
	do
		echo "${s} $line"
